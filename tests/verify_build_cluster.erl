...
confirm() ->
    ...
    %[Node1, Node2, Node3, Node4] = Nodes = rt:deploy_nodes(4, [{riak_core, [{handoff_concurrency, 128}]}]),
    ...
    ?assertEqual(ok, rt_sniffle:vm_register(Node1, ?UUID1, ?HV)),

    ?assertEqual({ok,
                  [{<<"hypervisor">>,?HV},
                   {<<"uuid">>,?UUID1}]},
                 rt_sniffle:vm_get(Node1, ?UUID1)),
...
wait_and_validate(RingNodes, UpNodes) ->
    ...
    [?assertEqual({ok,
                   [{<<"hypervisor">>,?HV},
                    {<<"uuid">>,?UUID1}]},
                  rt_sniffle:vm_get(Node, ?UUID1)) || Node <- UpNodes],
    done.
